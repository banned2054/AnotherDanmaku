...
public class GuiCommand
{
    ...
    public Dictionary<string, Action<IList<string>>> Commands => _commands ??=
        new Dictionary<string, Action<IList<string>>>
        {
            ...
            ["open-optical-media"] = OpenDvdOrBdFolder,
            ...
            ["show-audio-devices"] = _ => Msg.ShowInfo(Player.GetPropertyOsdString("audio-device-list")),
            ...
            ["show-properties"]    = _ => ShowProperties(),
    ...
    private static void OpenDvdOrBdFolder(IList<string> args)
    {
    ...
    private void ShowMediaInfo(IList<string> args)
    {
        ...
        if (File.Exists(path) && osd)
        {
            ...
            if (FileTypes.Image.Contains(path.Ext()))
            {
