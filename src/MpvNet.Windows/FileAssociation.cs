...
public static class FileAssociation
{
    public static void Register(string perceivedType, string[] extensions)
    {
        ...
        var exeFilenameNoExt = Path.GetFileNameWithoutExtension(exePath);
        ...
        if (perceivedType != "unreg")
        {
            ...
            RegistryHelp.SetValue(@"HKCU\Software\Microsoft\Windows\CurrentVersion\App Paths\" + exeFilename, "",
                                  exePath);
            RegistryHelp.SetValue(@"HKCR\Applications\" + exeFilename, "FriendlyAppName", "mpv.net media player");
            RegistryHelp.SetValue(@"HKCR\Applications\" + exeFilename + @"\shell\open\command", "",
                                  $"\"{exePath}\" \"%1\"");
            ...
            RegistryHelp.SetValue(@"HKLM\SOFTWARE\RegisteredApplications", "mpv.net",
                                  @"SOFTWARE\Clients\Media\mpv.net\Capabilities");
            RegistryHelp.SetValue(@"HKLM\SOFTWARE\Clients\Media\mpv.net\Capabilities", "ApplicationDescription",
            ...
            foreach (var ext in extensions)
            {
                ...
                RegistryHelp.SetValue(@"HKCR\" + exeFilenameNoExt + "." + ext + @"\shell\open\command", "",
                                      $"\"{exePath}\" \"%1\"");
                RegistryHelp.SetValue(@"HKLM\SOFTWARE\Clients\Media\mpv.net\Capabilities\FileAssociations", "." + ext,
                                      exeFilenameNoExt + "." + ext);
            }
        }
        else
        {
            ...
            foreach (var id in Registry.ClassesRoot.GetSubKeyNames())
            {
