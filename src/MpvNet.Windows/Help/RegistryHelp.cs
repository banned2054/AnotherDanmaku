...
public static class RegistryHelp
{
    ...
    {
        using var regKey =
            GetRootKey(AppKey).CreateSubKey(AppKey[5..], RegistryKeyPermissionCheck.ReadWriteSubTree);
    ...
    {
        using var regKey =
            GetRootKey(path).CreateSubKey(path[5..], RegistryKeyPermissionCheck.ReadWriteSubTree);
        regKey.SetValue(name, value);
    }
    ...
    {
        using var regKey = GetRootKey(path).OpenSubKey(path[5..]);
    ...
    public static void RemoveValue(string path, string name)
    {
        try
        {
            using var regKey = GetRootKey(path).OpenSubKey(path[5..], true);
    ...
    private static RegistryKey GetRootKey(string path) => path[..4] switch
    {
