...
public static class InputHelp
{
    public static List<Binding> GetDefaults()
    {
        var bindings = new List<Binding>()
        {
            ...
            new(_("File"), _("Add files/URLs to playlist from clipboard"),
    ...
    public static List<Binding> GetBindingsFromContent(string content)
    {
        ...
        if (string.IsNullOrEmpty(content)) return bindings;
        foreach (var line in content.Split('\r', '\n'))
        {
            ...
            var binding = new Binding
            {
                Input = value[..value.IndexOf(" ", StringComparison.Ordinal)]
            };
            ...
            {
                binding.Comment =
                    value[(value.IndexOf(App.MenuSyntax, StringComparison.Ordinal) + App.MenuSyntax.Length)..].Trim();
                value = value[..value.IndexOf(App.MenuSyntax, StringComparison.Ordinal)];

                if (binding.Comment.Contains(';'))
                    binding.Comment = binding.Comment[(binding.Comment.IndexOf(";", StringComparison.Ordinal) + 1)..]
                                             .Trim();
            }
