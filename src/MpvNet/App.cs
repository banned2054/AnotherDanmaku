...
public class AppClass
{
    ...
    {
        return IsStoreVersion ? "" : $" ({File.GetLastWriteTime(path).ToShortDateString()})";
    }

    private static bool IsStoreVersion => Folder.Startup.Contains("FrankSkare.mpvnet");
    ...
    public bool ProcessProperty(string name, string value, bool writeError = false)
    {
        switch (name)
        {
            case "audio-file-extensions" :
                FileTypes.Audio = value.Split(" ,;".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                return true;
            ...
            case "debug-mode" :
                DebugMode = value == "yes";
                return true;
            case "image-file-extensions" :
                FileTypes.Image = value.Split(" ,;".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                return true;
            case "language" :
                Language = value;
                return true;
            ...
            case "start-size" :
                StartSize = value;
                return true;
            case "video-file-extensions" :
                FileTypes.Video = value.Split(" ,;".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                return true;
    ...
    public void ApplyInputDefaultBindingsFix()
    {
        ...
        {
            var content = File.ReadAllText(Player.ConfPath);
