...
internal class GlobalHotKey
{
    ...
    {
        _hwnd = hwnd;
        var path = Player.ConfigFolder + "global-input.conf";
    ...
    private static void ProcessGlobalHotKeyLine(string line)
    {
        var key     = line[..line.IndexOf(" ", StringComparison.Ordinal)];
        var command = line[(line.IndexOf(" ", StringComparison.Ordinal) + 1)..];
        var parts   = key.Split('+');
        ...
        if (key.Length == 1)
        {
            var result = VkKeyScanEx(key[0], GetKeyboardLayout(0));
        ...
        if (vk <= 0) return;
        Commands[_id] = command.Trim();
        var success = RegisterHotKey(_hwnd, _id++, mod, vk);
    ...
    private static int Mpv_to_VK(string value)
    {
